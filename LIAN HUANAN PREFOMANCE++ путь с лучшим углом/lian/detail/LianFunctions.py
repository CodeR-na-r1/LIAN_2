...
def Expand(currentPoint: StagePoint, distanceDelta: int, end: StagePoint, angleDelta: int, mapPath: Dict[Point, Point], mapDistances: Dict[Point, int], mapAngles: Dict[Point, int], CLOSE: List[StagePoint], map: Map):
    ...
    if distanceBetweenPoints(currentPoint.point, end.point) < distanceDelta and end.point not in points:
    ...
    for point in points:
        ...
        if not map.isFree(point):
            continue
        angle = 0
        if currentPoint.point in mapPath and mapPath[currentPoint.point] != None:
        ...
        stagePoint = StagePoint(point, currentPoint.distance + distanceBetweenPoints(currentPoint.point, point), mapAngles[currentPoint.point] + angle) # ? parent Point or StagePoint

        if stagePoint in CLOSE:
            if stagePoint.angle < mapAngles[point]:
